...
export const isActiveToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (err.name === 'TokenExpiredError') {
            try {
                const decoded = jwt.verify(refreshToken, config.jwtRefreshSecret) as {id: string, role: string}; // Verify the refresh token
                if (!decoded) {
                    return next(new HttpError('Invalid refresh token', 401)); // If the refresh token is invalid, return an error
                }

                const refreshTokenDoc = await RefreshToken.findOne({ token: refreshToken, userId: decoded.id, revoked: false });
                if (!refreshTokenDoc) { 
                ...
                if (refreshTokenDoc) { // If the refresh token is found, revoke it because new tokens will be issued
                ...
                res.cookie("accessToken", newAccessToken, { // Set the new access token as a cookie
                    httpOnly: true,
                    secure: config.nodeEnv === 'production',
                    maxAge: 15 * 60 * 1000 // Set the cookie to expire in 15 minutes
                });

                res.cookie("refreshToken", newRefreshToken, { // Set the new refresh token as a cookie
                    httpOnly: true,
                    secure: config.nodeEnv === 'production',
